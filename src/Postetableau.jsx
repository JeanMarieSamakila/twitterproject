...
export default function Postetableau({ props }) {
  console.log(props);
  const tableau = {
  ...
  const [imglike, setImglike] = useState(false)
    ...
    const likeIcon = imglike ? 'images/heart-solid.svg' : 'images/React.svg';
    //    const LikeIcon=state ? BsFillHeartFill :BsHeart

  return (
    <div className="flex border-b  border-slate-800 ">
      <div className="ml-8 mt-5">
        <Image className=" w-13 h-13 rounded-full items-center" src={props.author_avatar}
          alt=""
        />
      </div>
      <div className="ml-8 mr-4 mt-5  ">
        <div className="flex ">
          <h2 className="text-white">{props.source}</h2>
          <Image className='w-8 h-8' src="images/Verified.svg" alt="" />
          <Paragraphe text={props.date} />
        </div>
        <Paragraphe text={props.text} />
        {props.image && 
          <Image
            src={props.image}
            className=" w-full objectif-cover h-60 rounded-3xl" 
            alt=""
          />
        }
        <div className="flex gap-12 mt-3 mb-3" >
          <div className="flex">
            <Image src="images/Reply.svg" alt="" />
            <Paragraphe text={props.replies} />
          </div>
          <div className="flex">
            <Image src="images/Retweet.svg" alt="" />
            <Paragraphe className="text-white" text={props.retweets } />
          </div>
          <div className="flex ">
          
            <img className="" onClick={handleclick} src={likeIcon} alt=""/>
            {/* <p className="text-white pt-1">{valeur}</p> */}
            <p className="text-white pt-1">{like}</p>
